...
This is how storefronts reach app-defined collections: `/apps/<app_id>/<collection>` and `/apps/<app_id>/<collection>/<id>`. The model must declare public permissions, and those permissions decide the behavior:

- **`scope: 'account'`** makes the collection customer-owned. Reads are auto-filtered to the logged-in account — do **not** add your own `account_id` filter — creates are stamped with it, and updates/deletes re-fetch the record scoped to the caller and 404 if it isn't theirs. GET, PUT and DELETE reject with `code: 'UNAUTHORIZED'` (`You must be logged in to access '<model>'`) when logged out; **POST does not** — it is exempt from that gate, so an anonymous create is accepted and stamped with an undefined `account_id`, landing a record nobody can read back. Scope is also only picked up off the model when `public_permissions.input` is present, so a scoped *read-only* collection silently stays world-readable. Without scope the collection is world-readable, which is what a public reviews list wants.
- **`methods` is a key-level permission, not something an app model can declare.** The gateway copies only `fields`, `query`, `expands`, `input.fields` and `scope` off a model; `methods` lives on a `clients:keys` record's own `permissions` — it is how `carts`, `accounts` and `payments` are blocked on the generic routes, and a verb it blocks rejects with `This action is not permitted`. So `input.fields` — required for any write, or you get `You may not update <model> without public permissions`, with fields outside the list rejected rather than ignored — opens **PUT and DELETE alongside POST** to every anonymous visitor, and an app collection has no way to close them. A DELETE carries no body, so the field check passes vacuously: `DELETE /apps/<app_id>/reviews/<id>` from any browser removes someone else's record. Own the records with `scope: 'account'`, or omit `input` entirely and take submissions through an app route function that checks `req.session?.account_id` (§VIII).

App **extension fields** are not reachable this way at all: `$app.<app_id>.*` written onto a standard model is invisible to storefront callers, because the storefront field allowlist is fixed server-side (the `products` list contains no `$app` entry) and is widened only by the public key record's own `permissions`. Surface app data through an app collection or a route function, not through a `$app` field on a product.
